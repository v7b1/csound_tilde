...
m_sym(c74::max::gensym((char*)m_name.c_str()))
{
}
...
ChannelGroup::ChannelGroup(c74::max::t_object *o, CSOUND *c, Direction d) :
...
void ChannelGroup::PrintChannels(void * outlet)
{
	...
	if(m_direction == IN_)
	{
        atom_setsym(&b[0], c74::max::gensym("IN"));
	}
	else
	{
        atom_setsym(&b[0], c74::max::gensym("OUT"));
	}
	...
	for(it = m_channels.begin(); it != m_channels.end(); it++)
	{
		...
		{
			atom_setsym(&b[0], it->m_sym);
            atom_setsym(&b[1], c74::max::gensym((char*)it->m_str_value.c_str()));
		}
