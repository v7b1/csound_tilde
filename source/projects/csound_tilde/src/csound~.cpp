...
void ext_main(void* r)
{
    ...
    class_addmethod(c, (method)csound_notify,       "notify", A_CANT, 0);
...
//	for(i=1; i<=totalVectors; i++) perform_args[i] = (void*) sp[i-1]->s_vec;
    ...
    x->vectorSize = (int)maxvectorsize;
...
void *csound_new(t_symbol *s, short argc, t_atom *argv)
{
	...
	try
	{
		...
		for(i = 0; i < argc; i++)
		{
			switch(argv[i].a_type)
			{
				...
				case A_FLOAT:
					...
					break;
				case A_SYM:
					str = (char *)argv[i].a_w.w_sym->s_name;
		...
		if(0 == path_topotentialname(x->cso->m_defaultPathID, "", tmpStr, false))
		{
            ...
            // vb-- hm, better try with path_nameconform
            char tmp[MAX_STRING_LENGTH];
            path_nameconform(tmpStr, tmp, PATH_STYLE_SLASH, PATH_TYPE_BOOT);
            //object_post(NULL, "conformed path: %s", tmp);
            SetEnvironment(x, tmp);
		#endif
			//SetEnvironment(x, x->cso->m_defaultPath.c_str());
...
// vb-- add a buffer notify method here.
t_max_err csound_notify(t_csound *x, t_symbol *s, t_symbol *msg, void *sender, void *data)
{
...
void SetEnvironment(t_csound *x, const char *path)
{
    ...
    object_post(x->m_obj, "path: %s", tmp);
	if(strlen(tmp))
	{
...
{
    // vb
    //object_warn((t_object *)x, "loadsamp message is currently not functional!");
	...
	tableNum = (int)atom_getlong(&argv[0]);
	channel = (int)atom_getlong(&argv[1]);
	...
	if(cso->m_compiled && !cso->m_performanceFinished)
	{
        ...
        object_post(NULL, "filename: %s", filename);
...
void csound_readbuf(t_csound *x, t_symbol *s, short argc, t_atom *argv)
{
	...
	{
        // vb, new buffer api
        t_symbol *bufname = atom_getsym(&argv[2]);
        if(!x->bufref)
            x->bufref = buffer_ref_new((t_object *)x, bufname);
        else
            buffer_ref_set(x->bufref, bufname);
        
        if(buffer_ref_exists(x->bufref)) {
            ...
            ct.ReadBufferTildeNew(x->bufref, abs(tableNum),channel,(tableNum < 0 ? 1 : 0),offsetSeconds,sizeSeconds);
        }
        else
            object_error(x->m_obj, "No buffer~ with name %s exists.", bufname);
	}
}
...
void csound_writebuf(t_csound *x, t_symbol *s, short argc, t_atom *argv)
{
	...
	if(cso->m_compiled && !cso->m_performanceFinished)
	{
        ...
        if(buffer_ref_exists(x->bufref)) {
            ...
            ct.WriteBufferTildeNew(x->bufref, abs(tableNum), channel, offsetSeconds, sizeSeconds);
