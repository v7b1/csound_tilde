...
void Args::SetCsoundArguments(short argc, const t_atom *argv, const string & path, const string & defaultPath)
{
	...
	try
	{
		...
		for(i=0; i<argc; i++)
		{
			...
			{
                atom_getsym(&argv[i]);
				str = (char *)argv[i].a_w.w_sym->s_name;    // vb-- have to cast to char*
					...
					else
					{
						...
						if(result == 0)
						{
							...
							#elif MACOSX
							{
                                 ...
                                 */
                                // vb-- try with path_nameconform
                                path_nameconform(tmp2, tmp, PATH_STYLE_SLASH, PATH_TYPE_BOOT);
                                std::cout << "final path: " << tmp << "\n";
							}
							#endif
