...
{
    memset(m_buffer, 0, sizeof(c74::max::t_atom) * m_capacity);
#ifdef _DEBUG
	m_largest_size = 0;
#endif
}
...
bool atom_buffer::add(c74::max::t_symbol *s)
{
	if(full()) return false;
...
{
	if(full()) return false;
    atom_setsym(&m_buffer[m_size++], c74::max::gensym((char*)s)); // Must force cast to char*.
	return true;
}
