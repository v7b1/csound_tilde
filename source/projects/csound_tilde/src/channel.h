...
private:
  ...
  MYFLT *m_csoundChanPtr; // A pointer to the actual Csound channel.
    c74::max::t_symbol *m_sym;           // Since gensym() is expensive, store symbol version of m_name here.
...
public:
    ChannelGroup(c74::max::t_object *o, CSOUND *c, Direction d);
	~ChannelGroup();
...
private:
    ...
    c74::max::t_object *m_obj;          // Used when calling object_post() or object_error().
