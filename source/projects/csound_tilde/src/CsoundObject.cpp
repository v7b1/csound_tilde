...
{
    printf("delete m_csound...\n\n");

	csoundDestroy(m_csound);
    //m_msg_buf.post();     // vb: causes max to hang when quitting. had to uncomment this line
}				
...
//			}
            ...
            //if(CSOUND_SUCCESS != csoundCompile(m_csound)) {
				...
				if(!m_performanceFinished)
				{
					...
					while(m_out_index < m_ksmps && j < vectorSize)
					{
						...
						for(chan=0; chan<m_outChans; chan++)
							out[chan][j] = (float)(csOut[indexMultChans + chan] * m_oneDivScale);
						++j;
						++m_out_index;
					}
				}
		...
		{	/* ksmps does not evenly divide the current Max vector size. Here's a description of the loop: 
			...
			for(i=0; i<vectorSize; i++)
			{
				if(!m_performanceFinished)
				{
					...
					for(chan=0; chan<m_outChans; chan++)
						out[chan][i] = (float)(csOut[indexMultChans + chan] * m_oneDivScale);
			...
			for(i=0; i<vectorSize; i++)
			{
				if(!m_performanceFinished)
				{
					...
					while(m_out_index < m_ksmps && j < vectorSize)
					{
						indexMultChans = m_out_index * m_chans;
						for(chan=0; chan<m_outChans; chan++)
							out[chan][j] = (double)(csOut[indexMultChans + chan] * m_oneDivScale);
						++j;
						++m_out_index;
					}
				}
		...
		{	/* ksmps does not evenly divide the current Max vector size. Here's a description of the loop: 
			...
			for(i=0; i<vectorSize; i++)
			{
				if(!m_performanceFinished)
				{
					...
					for(chan=0; chan<m_outChans; chan++)
						out[chan][i] = (double)(csOut[indexMultChans + chan] * m_oneDivScale);
