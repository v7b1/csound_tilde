...
int CsoundTable::WriteBufferTilde(t_symbol *bufferSymbol, int num, int targetChannel, float offsetTime, float sizeTime)
{
    ...
    t_buffer_obj bb = (t_object *)m_obj->bufref;
    //t_buffer_obj bb;
...
int CsoundTable::WriteBufferTildeNew(t_buffer_ref *bufref, int num, int targetChannel, float offsetTime, float sizeTime)
{
    ...
    t_buffer_obj *b = buffer_ref_getobject(bufref);
    
    buffer = buffer_locksamples(b);
    if (!buffer) {
        object_error(m_obj, "Can't lock buffer~");
        return -1;
    }
    
    frames = buffer_getframecount(b);
    nchls = buffer_getchannelcount(b);
    
    MYFLT csoundSR = csoundGetSr(m_csound);
    

    
    if(targetChannel < 0 || targetChannel > nchls)
    {
        //object_error(m_obj, "buffer~ object %s doesn't have a channel #%d.", bufferSymbol->s_name, targetChannel);
        object_error(m_obj, "buffer~ object %s doesn't have a channel #%d.", "buffername", targetChannel);
    ...
    // If sizeFrames is greater than the # of frames in the target buffer~,
    // then sizeFrames = target buffer~'s frame size.
    if(sizeFrames > frames) sizeFrames = frames;
    
    // Copy the audio data.
    if(targetChannel == 0)
        for(i=0; i<(sizeFrames * nchls); i++)
            buffer[i] = m_data[i + offsetSamples];
    else
    {
        tmp = targetChannel-1;
        for(i=0; i<sizeFrames; i++)
            buffer[i*nchls + tmp] = m_data[i + offsetFrames];
    }
    
    buffer_unlocksamples(b);     // Release the buffer~ object.
    return 0;
}


int CsoundTable::ReadBufferTildeNew(t_buffer_ref *bufref, int num, int sourceChannel, int resize, float offsetTime, float sizeTime)
{
    char eventStr[MAX_EVENT_MESSAGE_SIZE];
    ...
    if(!GetTable(num))
    {   // num doesn't exist.
        if(sourceChannel > 0 && sourceChannel > nchnls) sourceChannel = nchnls;
        if(sourceChannel > 0) size = frames;
        else                  size = frames * nchnls;
        
        // If offsetTime or sizeTime is positive, then they are in seconds.  Otherwise, they are in frames.
        if(offsetTime >= 0)    offsetFrames = (int)(offsetTime * (float) b_sr);
        else                offsetFrames = (int)-offsetTime;
        if(offsetFrames >= frames)
        {
            object_error(m_obj, "Offset time is greater than size of buffer~");
            buffer_unlocksamples(b);
            return 1;
        }
        ...
        else if(sizeTime > 0.0f)
        {
            // User has specified the amount of frames to read in seconds.
            // If necessary, change sizeFrames so that (offsetFrames + sizeFrames) <= sf.frames.
            sizeFrames = (int)(sizeTime * (float) b_sr);
            if((offsetFrames + sizeFrames) > frames)
                sizeFrames = frames - offsetFrames;
        }
        ...
        else                  m_samples = sizeFrames * nchnls;
        m_samples = nextPowOf2(m_samples) + 1; // Add guard point for interpolating opcodes.
        ...
        if(GetTable(num))
        {
            ...
            {
                // Copy all channels as interleaved data.
                tmp = offsetFrames * nchnls;
                for(i=0; i<(sizeFrames * nchnls); i++) m_data[i] = buffer[i+tmp];
            }
            else
            {
                // Copy only the specified channel's data.
                tmp = sourceChannel-1+(offsetFrames * nchnls);
                for(i=0; i<sizeFrames; i++) m_data[i] = buffer[i*nchnls + tmp];
            }
        }
        buffer_unlocksamples(b);
    }
    else // num exists
    {
        ...
        if(offsetTime >= 0)    offsetFrames = (int)(offsetTime * b_sr);
        else                offsetFrames = (int)-offsetTime;
        if(offsetFrames >= frames)
        {
            object_error(m_obj, "Offset time is greater than size of buffer~.");
        ...
        else if(sizeTime > 0.0f)
        {
            // User has specified the amount of frames to read in seconds.
            // If necessary, change sizeFrames so that (offsetFrames + sizeFrames) <= sf.frames.
            sizeFrames = (int)(sizeTime * b_sr);
            if((offsetFrames + sizeFrames) > frames)
                sizeFrames = frames - offsetFrames;
        }
        
        if(sourceChannel > 0) size = sizeFrames;
        else                  size = sizeFrames * nchnls;
        ...
        if(resize)
        {
            ...
            if(!GetTable(num)) { buffer_unlocksamples(b); return 2; }
        }
        else if(size > m_samples)
        {
            size = m_samples;
            if(sourceChannel > 0) sizeFrames = m_samples;
            else                  sizeFrames = m_samples / nchnls;
        }
